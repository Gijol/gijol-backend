...
public class ArtAndSport {
    ...
    private static void checkSportMandatoryNumber(EtcMandatory etcMandatory, List<TakenCourse> takenCourses, int studentId) {
        ...
        if (studentId < 20 && courseCount < fourMandatory) {
            etcMandatory.addMessage(String.format("체육 과목(%s)을 %d개 더 수강해야 합니다.", SPORT_CODE, fourMandatory - courseCount));
        }

        if (studentId >= 20 && courseCount < twoMandatory) {
            etcMandatory.addMessage(String.format("체육 과목(%s)을 %d개 더 수강해야 합니다.", SPORT_CODE, twoMandatory - courseCount));
        }
    }
    ...
    private static void checkArtsMandatoryNumber(EtcMandatory etcMandatory, List<TakenCourse> takenCourses, int studentId) {
        ...
        int courseCount = takenCourses.size();
        if (studentId < 20 && courseCount < fourMandatory) {
            etcMandatory.addMessage(String.format("예능 과목(%s)을 %d개 더 수강해야 합니다.", ART_CODE, fourMandatory - courseCount));
        }

        if (studentId >= 20 && courseCount < twoMandatory) {
            etcMandatory.addMessage(String.format("예능 과목(%s)을 %d개 더 수강해야 합니다.", ART_CODE, twoMandatory - courseCount));
        }
    }
}
